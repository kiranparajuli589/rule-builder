import type { RuleDTO, ConditionDTO } from "@/types/rule-builder";
import { JoinOperator } from "@/types/rule-builder";
...
export const RuleService = {
	...
	hasCircularDependency(conditions: ConditionDTO[]): boolean {
		...
		for (const condition of flatConditions) {
			if (!condition.field || !condition.operator || condition.isGroup) continue;
			...
			if (condition.operator === "==" && existingValues.size > 0 && !existingValues.has(condition.value || "")) {
	...
	cleanRuleForExport(rule: RuleDTO): RuleDTO {
		const cleanConditions = (conditions: ConditionDTO[]): ConditionDTO[] => conditions.map(condition => {
			const cleaned: any = {
